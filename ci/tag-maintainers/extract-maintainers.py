...
def extract_maintainers(changed_files: List[str], pr_author: str) -> List[str]:
    ...
    # Convert changed files to Nix list format
    changed_files_nix = '[ ' + ' '.join(f'"{f}"' for f in changed_files) + ' ]'

    # Nix expression to directly evaluate maintainers for the intersection of changed files
    nix_expr = f"""
    let
      nixvim = import ./.;
    ...
    # Get maintainers from Nix evaluation
